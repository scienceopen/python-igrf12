...
def IGRF13(lat, lon, alt, dates, isv=0, itype=1):
    ...
    Output:
    [[81.27769572 52.32187034]
     [ 3.42999051  3.554509  ]
     [18.78557498 23.96335595]
     [83.49087665 57.65808771]]
    ...
    for k in range(1, max_kmx):
        ...
        m = int_v(np.where(reset_mask, 0, m))
        n = int_v(np.where(reset_mask, n+1, n))
        ...
        j = int_v(np.where(equal_and_not2_mask, k-n-1, 0))
        ...
        i = int_v(np.where(notequal_mask, k - n, 0)) 
        ...
        B_east += np.where(noteuqal0_mask & ~less_mask, 
                           ...
                           0)
        l += int_v(np.where(noteuqal0_mask, 2, 1))
        m += 1

    one = B_north
    B_north = B_north*cd + B_vert*sd
    B_vert = B_vert - one*sd
